...
class PyInstArchive:
    ...
    def checkFile(self):
        logger.debug('[+] Processing {0}'.format(self.filePath))
    ...
    def getCArchiveInfo(self):
        ...
        logger.debug('[+] Python version: {0}.{1}'.format(self.pymaj, self.pymin))
        ...
        logger.debug('[+] Length of package: {0} bytes'.format(lengthofPackage))
        return True
    ...
    def parseTOC(self):
        ...
        while parsedLen < self.tableOfContentsSize:
            ...
            try:
                ...
                logger.debug('[!] Warning: File name {0} contains invalid bytes. Using random name {1}'.format(name, newName))
                name = newName
            ...
            if len(name) == 0:
                name = str(uniquename())
                logger.debug('[!] Warning: Found an unamed file in CArchive. Using random name {0}'.format(name))
            ...
            parsedLen += entrySize
        logger.debug('[+] Found {0} files in CArchive'.format(len(self.tocList)))
    ...
    def extractFiles(self, output_dir:str):
        logger.debug('[+] Beginning extraction...please standby')
        extractionDir = os.path.join(os.getcwd(), output_dir)
        ...
        for entry in self.tocList:
            ...
            if entry.cmprsFlag == 1:
                try:
                    ...
                    logger.error('[!] Error : Failed to decompress {0}'.format(entry.name))
                    continue
            ...
            if entry.typeCmprsData == b's':
                ...
                logger.debug('[+] Possible entry point: {0}.pyc'.format(entry.name))
    ...
    def _extractPyz(self, name, output):
        dirName =  output
        ...
        with open(name, 'rb') as f:
            ...
            if self.pycMagic == b'\0' * 4:
                ...
                self.pycMagic = pyzPycMagic
                logger.debug('[!] Warning: pyc magic of files inside PYZ archive are different from those in CArchive')

            # Skip PYZ extraction if not running under the same python version
            if self.pymaj != sys.version_info.major or self.pymin != sys.version_info.minor:
                logger.debug('[!] Warning: This script is running in a different Python version than the one used to build the executable.')
                logger.debug('[!] Please run this script in Python {0}.{1} to prevent extraction errors during unmarshalling'.format(self.pymaj, self.pymin))
                logger.debug('[!] Skipping pyz extraction')
                return
            ...
            try:
                ...
                logger.debug('[!] Unmarshalling FAILED. Cannot extract {0}. Extracting remaining files.'.format(name))
                return

            logger.debug('[+] Found {0} files in PYZ archive'.format(len(toc)))
    ...
    def parse_PYC(self):
        ...
        for file in os.listdir(self.__OUTPUT):
            try:
                ...
                _cmd = f"\"{os.path.join(self.__RUN_DIR, 'bin\\pycdas.exe')}\" \"{os.path.join(self.__OUTPUT, file)}\""
                logger.debug(_cmd)
